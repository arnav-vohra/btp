...
Host::Host()
{
	...
	t_arrival=random_num(80,120);						//seconds
	t_departure=random_num(8000,24000);
...
int main()
{
	...
	{	
		...
		{
			sumWeight+= 1.0* (scheduledTasks[i].task_length/meanFRes)/ (scheduledTasks[i].deadline - scheduledTasks[i].t_arrival);// -2*client_list[i].task_length/meanDataRate);
		}
		...
		if(iterations!=1)
		{
			if(systemLoad<targetSystemLoad-0.10)
			{
				c = (targetSystemLoad*sumCapacity - sumWeight)*0.8;
				cout<<endl<<c<<" clients created\n";
				h = 0;
			}
			else if(systemLoad>targetSystemLoad+0.10)
			{
				c = 0;
				h = (sumWeight/targetSystemLoad- sumWeight/systemLoad)/6.00;
